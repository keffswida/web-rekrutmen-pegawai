<link rel="stylesheet" href="https://unpkg.com/aos@next/dist/aos.css" />
<script src="/assets/js/aos.js"></script>

<footer class="bg-gray-800 text-white pt-12 pb-8 font-poppins">
    <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">

            <div class="flex justify-center md:justify-start">
                <img src="{{ asset('assets/images/logo_rsi.png') }}" alt="Logo" class="w-24 h-24 md:w-32 md:h-32">
            </div>

            <div>
                <h2 class="text-lg md:text-xl font-light mb-4">QUICK MENU</h2>
                <ul class="space-y-2">
                    <li><a href="/about" class="hover:text-green-400">Tentang Kami</a></li>
                    <li><a href="/career" class="hover:text-green-400">Cari Lowongan</a></li>
                </ul>
            </div>

            <div>
                <h2 class="text-lg md:text-xl font-light mb-4">BAGIAN SDM</h2>
                <ul class="space-y-3">
                    <li class="flex flex-col md:flex-row items-center md:items-start space-x-0 md:space-x-2">
                        <svg viewBox="0 0 24 24" class="h-6 w-6 mt-1 flex-shrink-1" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M12 2C7.58172 2 4 6.00258 4 10.5C4 14.9622 6.55332 19.8124 10.5371 21.6744C11.4657 22.1085 12.5343 22.1085 13.4629 21.6744C17.4467 19.8124 20 14.9622 20 10.5C20 6.00258 16.4183 2 12 2ZM12 12C13.1046 12 14 11.1046 14 10C14 8.89543 13.1046 8 12 8C10.8954 8 10 8.89543 10 10C10 11.1046 10.8954 12 12 12Z"
                                fill="white" />
                        </svg>
                        <span class="font-light">RSI Surabaya Jemursari, Jl. Jemursari 51-57 Surabaya</span>
                    </li>
                    <li class="flex flex-col md:flex-row items-center md:items-start space-x-0 md:space-x-2">
                        <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M10.0376 5.31617L10.6866 6.4791C11.2723 7.52858 11.0372 8.90532 10.1147 9.8278C10.1147 9.8278 10.1147 9.8278 10.1147 9.8278C10.1146 9.82792 8.99588 10.9468 11.0245 12.9755C13.0525 15.0035 14.1714 13.8861 14.1722 13.8853C14.1722 13.8853 14.1722 13.8853 14.1722 13.8853C15.0947 12.9628 16.4714 12.7277 17.5209 13.3134L18.6838 13.9624C20.2686 14.8468 20.4557 17.0692 19.0628 18.4622C18.2258 19.2992 17.2004 19.9505 16.0669 19.9934C14.1588 20.0658 10.9183 19.5829 7.6677 16.3323C4.41713 13.0817 3.93421 9.84122 4.00655 7.93309C4.04952 6.7996 4.7008 5.77423 5.53781 4.93723C6.93076 3.54428 9.15317 3.73144 10.0376 5.31617Z"
                                fill="white" />
                            <path
                                d="M13.2595 1.88008C13.3257 1.47119 13.7122 1.19381 14.1211 1.26001C14.1464 1.26485 14.2279 1.28007 14.2705 1.28958C14.3559 1.30858 14.4749 1.33784 14.6233 1.38106C14.9201 1.46751 15.3347 1.60991 15.8323 1.83805C16.8286 2.2948 18.1544 3.09381 19.5302 4.46961C20.906 5.84541 21.705 7.17122 22.1617 8.1675C22.3899 8.66511 22.5323 9.07972 22.6187 9.3765C22.6619 9.5249 22.6912 9.64393 22.7102 9.72926C22.7197 9.77193 22.7267 9.80619 22.7315 9.8315L22.7373 9.86269C22.8035 10.2716 22.5286 10.6741 22.1197 10.7403C21.712 10.8063 21.3279 10.5303 21.2601 10.1233C21.258 10.1124 21.2522 10.083 21.2461 10.0553C21.2337 9.99994 21.2124 9.91212 21.1786 9.79597C21.1109 9.56363 20.9934 9.2183 20.7982 8.79262C20.4084 7.94232 19.7074 6.76813 18.4695 5.53027C17.2317 4.2924 16.0575 3.59141 15.2072 3.20158C14.7815 3.00642 14.4362 2.88889 14.2038 2.82122C14.0877 2.78739 13.9417 2.75387 13.8863 2.74154C13.4793 2.67372 13.1935 2.2878 13.2595 1.88008Z"
                                fill="white" />
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M13.4857 5.32954C13.5995 4.93126 14.0146 4.70065 14.4129 4.81444L14.2069 5.53558C14.4129 4.81444 14.4129 4.81444 14.4129 4.81444L14.4144 4.81485L14.4159 4.81529L14.4192 4.81627L14.427 4.81858L14.4468 4.82475C14.4618 4.82957 14.4807 4.83585 14.5031 4.84381C14.548 4.85975 14.6074 4.88242 14.6802 4.91362C14.8259 4.97605 15.0249 5.07248 15.2695 5.21719C15.7589 5.50687 16.4271 5.98805 17.2121 6.77302C17.9971 7.55799 18.4782 8.22618 18.7679 8.71564C18.9126 8.96015 19.009 9.15922 19.0715 9.3049C19.1027 9.37771 19.1254 9.43707 19.1413 9.48198C19.1493 9.50443 19.1555 9.52326 19.1604 9.53834L19.1665 9.55812L19.1688 9.56587L19.1698 9.56921L19.1703 9.57074C19.1703 9.57074 19.1707 9.57218 18.4495 9.77822L19.1707 9.57218C19.2845 9.97046 19.0538 10.3856 18.6556 10.4994C18.2607 10.6122 17.8492 10.3864 17.7313 9.99437L17.7276 9.98359C17.7223 9.96857 17.7113 9.93898 17.6928 9.89578C17.6558 9.80944 17.5887 9.66821 17.4771 9.47962C17.2541 9.10288 16.8514 8.53363 16.1514 7.83368C15.4515 7.13373 14.8822 6.73102 14.5055 6.50805C14.3169 6.39644 14.1757 6.32934 14.0893 6.29234C14.0461 6.27382 14.0165 6.26279 14.0015 6.25746L13.9907 6.25376C13.5987 6.13588 13.3729 5.72443 13.4857 5.32954Z"
                                fill="white" />
                        </svg>
                        <span class="font-light">031 - 8471877 Ext. 8333</span>
                    </li>
                    <li class="flex flex-col md:flex-row items-center md:items-start space-x-0 md:space-x-2">
                        <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M3.17157 5.17157C2 6.34315 2 8.22876 2 12C2 15.7712 2 17.6569 3.17157 18.8284C4.34315 20 6.22876 20 10 20H14C17.7712 20 19.6569 20 20.8284 18.8284C22 17.6569 22 15.7712 22 12C22 8.22876 22 6.34315 20.8284 5.17157C19.6569 4 17.7712 4 14 4H10C6.22876 4 4.34315 4 3.17157 5.17157ZM18.5762 7.51986C18.8413 7.83807 18.7983 8.31099 18.4801 8.57617L16.2837 10.4066C15.3973 11.1452 14.6789 11.7439 14.0448 12.1517C13.3843 12.5765 12.7411 12.8449 12 12.8449C11.2589 12.8449 10.6157 12.5765 9.95518 12.1517C9.32112 11.7439 8.60271 11.1452 7.71636 10.4066L5.51986 8.57617C5.20165 8.31099 5.15866 7.83807 5.42383 7.51986C5.68901 7.20165 6.16193 7.15866 6.48014 7.42383L8.63903 9.22291C9.57199 10.0004 10.2197 10.5384 10.7666 10.8901C11.2959 11.2306 11.6549 11.3449 12 11.3449C12.3451 11.3449 12.7041 11.2306 13.2334 10.8901C13.7803 10.5384 14.428 10.0004 15.361 9.22291L17.5199 7.42383C17.8381 7.15866 18.311 7.20165 18.5762 7.51986Z"
                                fill="white" />
                        </svg>
                        <span class="font-light">sdm@rsisjs.id</span>
                    </li>
                    <li class="flex flex-col md:flex-row items-center md:items-start space-x-0 md:space-x-2">
                        <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12Z"
                                fill="white" />
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M12 7.25C12.4142 7.25 12.75 7.58579 12.75 8V11.6893L15.0303 13.9697C15.3232 14.2626 15.3232 14.7374 15.0303 15.0303C14.7374 15.3232 14.2626 15.3232 13.9697 15.0303L11.4697 12.5303C11.329 12.3897 11.25 12.1989 11.25 12V8C11.25 7.58579 11.5858 7.25 12 7.25Z"
                                fill="black" />
                        </svg>
                        <div>
                            <div class="font-light">Senin - Jumat : 08.00 – 15.00</div>
                            <div class="font-light">Sabtu : 08.00 – 13.00</div>
                        </div>
                    </li>
                </ul>
            </div>
        </div>

        <!-- Copyright Section -->
        <div class="text-center mt-8 pt-8 border-t border-gray-700 text-sm">
            <p>&copy; {{ date('Y') }} Web E-Recruitment RSI Surabaya Jemursari. Developed by Kefiar Sakki</p>
        </div>
    </div>
</footer>

<script>
    AOS.init();
</script>
